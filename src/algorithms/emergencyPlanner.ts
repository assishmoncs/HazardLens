...
export interface FacilityEdge { from: string; to: string; type: string; }
export interface RouteResult { workerId: string; path: string[]; distanceM: number; riskScore: number; estimatedSeconds: number; avoidedHazardCost: number; }
export interface RiskAssessment { score: number; level: 'LOW' | 'GUARDED' | 'HIGH' | 'CRITICAL'; contributors: string[]; }
export interface InterventionCandidate { name: string; apply: (runtime: SimulationRuntime) => void; }
export interface RankedIntervention { name: string; lossAvoidedUsd: number; crewDoseReduction: number; assetLossReduction: number; firesReduced: number; score: number; }
...
export class EmergencyPlanner {
  ...
  constructor(edges: FacilityEdge[]) { this.edges = edges; }
  ...
  safestRoute(snapshot: WorldSnapshot, workerId: string, muster: Vec3): RouteResult | undefined {
    ...
    while (true) {
      ...
      let best = Infinity;
      for (const [node, d] of dist) if (!visited.has(node) && d < best) { best = d; current = node; }
    ...
    return { workerId, path, distanceM: Number(geometricDistance.toFixed(1)), riskScore: Number(riskScore.toFixed(1)), estimatedSeconds: Number((geometricDistance / 1.4).toFixed(1)), avoidedHazardCost: Number(Math.max(0, weightedDistance - geometricDistance).toFixed(1)) };
  }
  ...
  rankInterventions(runtime: SimulationRuntime, candidates: InterventionCandidate[], horizonSec = 45): RankedIntervention[] {
    const baseline = runtime.clone(); baseline.run(horizonSec, 0.1); const base = baseline.snapshot();
    const baseLoss = this.loss(base), baseDose = this.maxDose(base), baseFires = base.twins.filter(t => t.kind === 'fire' && t.active).length, baseFailed = this.failedCount(base);
    return candidates.map(candidate => {
      const branch = runtime.clone(); candidate.apply(branch); branch.run(horizonSec, 0.1); const snap = branch.snapshot();
      const lossAvoided = Math.max(0, baseLoss - this.loss(snap));
      const doseReduction = Math.max(0, baseDose - this.maxDose(snap));
      const assetReduction = Math.max(0, baseFailed - this.failedCount(snap));
      const firesReduced = Math.max(0, baseFires - snap.twins.filter(t => t.kind === 'fire' && t.active).length);
      const score = lossAvoided * 10 + doseReduction * 2 + assetReduction * 8 + firesReduced * 6;
      return { name: candidate.name, lossAvoidedUsd: Number(lossAvoided.toFixed(2)), crewDoseReduction: Number(doseReduction.toFixed(1)), assetLossReduction: assetReduction, firesReduced, score: Number(score.toFixed(2)) };
    }).sort((a, b) => b.score - a.score);
  }

  private hazardIntensity(fire: TwinState, point: Vec3): number {
    const d = Math.max(2, distance(fire.position, point)); const mw = numberValue(fire, 'intensityMw', 'intensity') || 1;
    return Math.min(80, mw * 120 / (d * d));
  }
  private hazardPenalty(snapshot: WorldSnapshot, point: Vec3): number {
    const fireRisk = snapshot.twins.filter(t => t.kind === 'fire' && t.active).reduce((sum, fire) => sum + this.hazardIntensity(fire, point), 0);
    const releaseRisk = snapshot.twins.filter(t => t.kind === 'release' && t.active).reduce((sum, release) => sum + Math.min(40, 20 / Math.max(2, distance(release.position, point))), 0);
    return clamp((fireRisk + releaseRisk) / 20, 0, 8);
  }
  private assetFailurePenalty(snapshot: WorldSnapshot, id: string): number { const twin = snapshot.twins.find(t => t.id === id); return twin && failed(twin) ? 100 : 0; }
  private failedCount(snapshot: WorldSnapshot): number { return snapshot.twins.filter(failed).length; }
  private maxDose(snapshot: WorldSnapshot): number { return Math.max(0, ...snapshot.twins.filter(t => t.kind === 'worker').map(w => numberValue(w, 'thermalDose', 'exposure'))); }
